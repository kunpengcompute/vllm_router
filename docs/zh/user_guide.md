...
> **说明：** vLLM-Router作为vLLM的外挂插件使用，增加数据并行部署功能，故对实际硬件环境同样不做要求，能运行vLLM推理框架即可。上述鲲鹏920新型号处理器及沐曦的曦云C500 GPU硬件环境仅作为参考示例。
